		...
		for (Cash Tempcash : this.cash) {
			if(Tempcash.getUseState().equals(UseState.정상) && Tempcash.getRemainAmt() < 0) {
				...
				if(Tempcash.getRemainAmt() * -1 < remainCash) {
					System.out.println("캐시번호 \'" +Tempcash.getCashNo() +"\' 의 "+ Tempcash.getRemainAmt() +" 원 마이너스 금액을 상환했습니다.");
					...
					System.out.println("캐시번호 \'" +Tempcash.getCashNo() +"\' 의 "+remainCash+" 원 마이너스 금액을 상환했습니다.");

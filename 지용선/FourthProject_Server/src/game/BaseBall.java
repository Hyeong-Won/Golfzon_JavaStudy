...
public class BaseBall  implements Serializable{
	...
	transient int[] 	baseBallNum = {0, 0, 0};	// 맞춰야하는 배열	
	transient int[] 	userNum	 	= {0, 0, 0};	// 유저 입력 배열
	transient int 	strikeCnt	= 0; 			// 스트라이크 카운트 수
	transient int 	ballCnt		= 0; 			// 볼 카운트 수
	transient int 	tryCnt		= 0; 			// 도전 횟수
	transient int     MaxTryCnt   = 5;           // 최대 도전횟수
	transient int     retry        = 1;		    // 다시 도전 YN
	...
	public boolean start() {
		...
		while (strikeCnt < 3 && tryCnt < MaxTryCnt) 
		{
			...
			Main.orderRead(this.user.getUserName(), ++tryCnt + "번째 도전! 숫자를 입력해주세요. :D");
			...
			for (int i = 0; i < userNum.length; i++) 
			{
				...
				{
					Main.orderRead(this.user.getUserName(), "1 ~ 9의 수만 입력 가능합니다. :(");
					printUI(i);
				}
				...
				{
					Main.orderRead(this.user.getUserName(), "이미 입력한 숫자입니다. :(");
					printUI(i);
				}
			}
			...
			// 결과 출력
			Main.orderRead(this.user.getUserName(), "<< " + tryCnt + "번째 도전 결과 >>");
	
			// 3개 다 맞추면 승리
			if (strikeCnt == 3)
			{
				Main.orderRead(this.user.getUserName(), "*:*:*:*:*:*:*:*:*:*:*:*:*:*:*");
				Main.orderRead(this.user.getUserName(), "*     승!!! 리!!!!! XD      *\"");
				Main.orderRead(this.user.getUserName(), "*:*:*:*:*:*:*:*:*:*:*:*:*:*:*");
				strikeCnt = tryCnt = 0;
				return true;
			}
			...
			{
				Main.orderRead(this.user.getUserName(), strikeCnt == 0 ? (ballCnt == 0 ? "아웃!!!!!!! ^0^ㅋㅋㅋㅋ" : ballCnt + " 볼!") : (ballCnt == 0 ? strikeCnt + " 스트라이크!" : ballCnt + " 볼! / " + strikeCnt + " 스트라이크!"));
			}
			
			while(tryCnt == MaxTryCnt) {
				Main.orderRead(this.user.getUserName(), "1. 다시도전(천원 과금) 2. 종료");
		...
		Main.orderRead(this.user.getUserName(), "*:*:*:*:*:*:*:*:*:*:*:*:*:*:*");
		Main.orderRead(this.user.getUserName(), "*     패!!! 배!!!!! -_-     *");
		Main.orderRead(this.user.getUserName(), "*:*:*:*:*:*:*:*:*:*:*:*:*:*:*");
	...
	{
		Main.orderRead(this.user.getUserName(), num + 1 + "번째 수 : ");
